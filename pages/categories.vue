...
const fetchCategories = async () => {
  try {
    const response = await fetch('https://jubilant-prosperity-production.up.railway.app/api/v1/categories.php')
...
const saveCategory = async () => {
  try {
    if (showEditModal.value) {
      // Update category
      const response = await fetch(`https://jubilant-prosperity-production.up.railway.app/api/v1/categories.php/${formData.value.id}`, {
        method: 'PUT',
...
const deleteCategory = async (id) => {
  if (confirm('Are you sure you want to delete this category?')) {
    try {
      const response = await fetch(`https://jubilant-prosperity-production.up.railway.app/api/v1/categories.php/${id}`, {
        method: 'DELETE'
      })
      const data = await response.json()
      if (data.success) {
