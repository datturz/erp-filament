...
const fetchCustomers = async () => {
  try {
    const response = await fetch('https://jubilant-prosperity-production.up.railway.app/api.php/v1/customers')
...
const saveCustomer = async () => {
  try {
    if (showEditModal.value) {
      // Update customer
      const response = await fetch(`https://jubilant-prosperity-production.up.railway.app/api.php/v1/customers/${formData.value.id}`, {
        method: 'PUT',
...
const deleteCustomer = async (id) => {
  if (confirm('Are you sure you want to delete this customer?')) {
    try {
      const response = await fetch(`https://jubilant-prosperity-production.up.railway.app/api.php/v1/customers/${id}`, {
        method: 'DELETE'
      })
      const data = await response.json()
      if (data.success) {
