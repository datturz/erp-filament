...
const fetchProducts = async () => {
  try {
    const response = await fetch('https://jubilant-prosperity-production.up.railway.app/api.php/v1/products')
...
const saveProduct = async () => {
  try {
    if (showEditModal.value) {
      // Update product
      const response = await fetch(`https://jubilant-prosperity-production.up.railway.app/api.php/v1/products/${formData.value.id}`, {
        method: 'PUT',
...
const deleteProduct = async (id) => {
  if (confirm('Are you sure you want to delete this product?')) {
    try {
      const response = await fetch(`https://jubilant-prosperity-production.up.railway.app/api.php/v1/products/${id}`, {
        method: 'DELETE'
      })
      const data = await response.json()
      if (data.success) {
