...
const fetchSuppliers = async () => {
  try {
    const response = await fetch('https://jubilant-prosperity-production.up.railway.app/api.php/v1/suppliers')
...
const saveSupplier = async () => {
  try {
    if (showEditModal.value) {
      // Update supplier
      const response = await fetch(`https://jubilant-prosperity-production.up.railway.app/api.php/v1/suppliers/${formData.value.id}`, {
        method: 'PUT',
...
const deleteSupplier = async (id) => {
  if (confirm('Are you sure you want to delete this supplier?')) {
    try {
      const response = await fetch(`https://jubilant-prosperity-production.up.railway.app/api.php/v1/suppliers/${id}`, {
        method: 'DELETE'
      })
      const data = await response.json()
      if (data.success) {
